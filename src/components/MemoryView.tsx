  ...
  return (
    <div className="flex flex-col h-full bg-[var(--app-background)] rounded-lg shadow-sm overflow-hidden border border-[var(--border)]">
      <div className="bg-[var(--toolbar-background)] px-4 py-2 border-b border-[var(--border)]">
        <h3 className="text-sm font-bold uppercase tracking-wider text-[var(--app-foreground)] opacity-70">Memory (0x2000 - 0x2080)</h3>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {words.map((word, index) => {
              const addrNum = START_ADDR + (index * 4);
              const addr = addrNum.toString(16).padStart(4, '0').toUpperCase();
